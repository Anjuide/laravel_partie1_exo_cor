...
class SondageController extends Controller
{
	...
	public function create($nom)
	{
		...
		// On doit transmettre 2 paramètres à la vue : "sondage" 
		// pour les informations du sondage et "nom" pour le nom du sondage
		return View ('sondage',['nom' => $nom, 'sondage' => $this->sondageGestion->getSondage($nom)]); 
	}
	...
	public function store(SondageRequest $request, $nom)
	{
		...
		if($this->sondageGestion->save($nom, $request->all())) 
		{
			...
			return View ('resultats',['sondage' => $this->sondageGestion->getSondage($nom), 'resultats' => $this->sondageGestion->getResults($nom), 'nom' => $this->sondageGestion->getSondage($nom)['question']]);
		}
